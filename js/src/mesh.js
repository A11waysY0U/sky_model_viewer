...
function parseModern(raw, name, version, filename) {
  ...
  const payloadOffset = (version >= 0x20) ? 0x4e : 0x4a;
  const isCompressed = dv.getInt32(payloadOffset, true);
  const cs = dv.getInt32(payloadOffset + 4, true);
  const us = dv.getInt32(payloadOffset + 8, true);
  if (cs <= 0 || us <= 0 || payloadOffset + 12 + cs > raw.length) throw new Error('压缩数据边界无效');
